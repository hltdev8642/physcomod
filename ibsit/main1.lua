...
function tick()
	...
	for body = #list, 1, -1 do
		...
		if not time[body] and IsBodyBroken(body) and IsBodyActive(body) then
			...
			pos[body], pos[-body], ind = TransformToLocalPoint(GetBodyTransform(body), breakpoint), breakpoint, true
		end
	end
end
...
function update()
	...
	for body = #gcl, 1, -1 do
		...
		elseif val == "gc" then
			vel[body], time[body], pos[body], pos[-body] = RemoveTag(body, "spd"), nil, nil, nil
		end
	end
end
...
function postUpdate()
	...
	for body = #gcl, 1, -1 do
		...
		if val == "calc" then
			...
			if not pos[-body] then
				val = true
			elseif VecLength(VecSub(c, pos[-body])) < 0.125 then
				val = false
			else
				pos[-body] = nil
				val = true
